...
def sha512(message):
    ...
    while len(message) % 128 != 112:
        message += b'\x00'
    message += orig_len_in_bits.to_bytes(16, byteorder='big')

    # Process the message in 1024-bit blocks
    for i in range(0, len(message), 128):
        chunk = message[i:i+128]
        words = [int.from_bytes(chunk[j:j+8], byteorder='big') for j in range(0, 128, 8)]

        # Extend the sixteen 64-bit words into eighty 64-bit words
        for j in range(16, 80):
            s0 = right_rotate(words[j-15], 1) ^ right_rotate(words[j-15], 8) ^ (words[j-15] >> 7)
            s1 = right_rotate(words[j-2], 19) ^ right_rotate(words[j-2], 61) ^ (words[j-2] >> 6)
        ...
        for j in range(80):
            ...
            ch = (e & f) ^ ((~e) & g)
            t1 = (h + s1 + ch + K[j] + words[j]) & 0xFFFFFFFFFFFFFFFF
    ...
    return (h0.to_bytes(8, byteorder='big') +
            h1.to_bytes(8, byteorder='big') +
            h2.to_bytes(8, byteorder='big') +
            h3.to_bytes(8, byteorder='big') +
            h4.to_bytes(8, byteorder='big') +
            h5.to_bytes(8, byteorder='big') +
            h6.to_bytes(8, byteorder='big') +
            h7.to_bytes(8, byteorder='big'))

# Example usage
message = b"Hello, World!"
hashed_message = sha512(message)
print('The input message: Hello, World!')
print("SHA-512 Hash:", hashed_message.hex())
